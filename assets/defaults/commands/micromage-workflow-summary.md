...
**Reply with**: `@micromage do quick wins` to auto-fix these.
...
**Reply with**: `@micromage create follow-up issues` to create these.
...
**Reply with**: `@micromage update docs` to send a docs agent.
...
1. **Review the PR**: {url}
2. **Quick wins**: Reply `@micromage do quick wins` on PR (or skip)
3. **Create issues**: Reply `@micromage create follow-up issues` (or skip)
4. **Merge when ready**
