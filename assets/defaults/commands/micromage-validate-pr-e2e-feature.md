---
description: Start Micromage from the feature branch, use agent-browser to verify the fix works correctly
argument-hint: (none - reads from artifacts)
---

# E2E Testing: Feature Branch (Verify Fix)

Start Micromage from the **feature branch** (this worktree) and use browser automation to verify that the bug is fixed and the UI/UX is correct. Take screenshots as evidence.
...
- `taskkill //F //IM node.exe` or `taskkill //F //IM bun.exe` — this kills Claude Code, the Micromage server, and all other workflows
...
## Phase 2: Start Micromage on Feature Branch
...
cd "$WORKTREE_PATH" && PORT=$BACKEND_PORT bun run --filter @micromage/server dev > "$ARTIFACTS_DIR/.e2e-feature-backend.log" 2>&1 &
BACKEND_PID=$!
...
```bash
# 1. Open the Micromage UI (ALWAYS use --session)
FRONTEND_PORT=$(cat $ARTIFACTS_DIR/.frontend-port | tr -d '\n')
...
### 4.2 Stop Feature Branch Micromage (Cross-Platform)
...
- **MICROMAGE_STARTED**: Backend and frontend running on feature branch code
...
- **MICROMAGE_STOPPED**: Processes killed, ports freed — **VERIFY ports are free before finishing**
