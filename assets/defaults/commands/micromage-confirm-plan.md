...
Run micromage-plan-setup first.
```
...
Continue to `micromage-implement-tasks` to execute the plan.

{If STOP:}
Revise the plan to address blockers, then re-run `micromage-plan-setup`.
```
...
Proceed to `micromage-implement-tasks` to execute the plan.
```
...
3. **Re-run planning** with `/micromage-plan` to generate a fresh plan
