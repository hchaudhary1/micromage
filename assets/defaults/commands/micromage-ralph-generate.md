...
| `.micromage/ralph/{slug}/prd.md` exists | Existing PRD in ralph dir | Generate prd.json alongside it |
...
```bash
mkdir -p .micromage/ralph/{slug}
```

### 5.3 Write prd.md

**Output path**: `.micromage/ralph/{slug}/prd.md`
...
**Output path**: `.micromage/ralph/{slug}/prd.json`
...
```bash
git add .micromage/ralph/{slug}/
git commit -m "docs: add Ralph PRD for {feature name}"
```

**PHASE_5_CHECKPOINT:**
- [ ] `.micromage/ralph/{slug}/prd.md` written with all sections
- [ ] `.micromage/ralph/{slug}/prd.json` written with properly sized stories
...
**Feature**: {name}
**Directory**: `.micromage/ralph/{slug}/`
...
- **FILES_WRITTEN**: Both prd.md and prd.json exist in `.micromage/ralph/{slug}/`
